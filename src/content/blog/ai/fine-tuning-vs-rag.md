...
범용 LLM(ChatGPT, Claude 등)은 의학 지식이 있지만, **우리 병원**에 대해서는 모른다.
...
이런 "우리만의 지식"을 AI에 넣는 방법이 크게 두 가지다: **RAG**와 **파인튜닝**.

### 파인튜닝이란

파인튜닝(Fine-tuning)은 이미 학습된 LLM을 **추가 데이터로 재학습**시키는 것이다.
...
[RAG(Retrieval-Augmented Generation)](/blog/ai/what-is-rag/)는 질문이 들어올 때마다 **외부 데이터베이스에서 관련 문서를 검색**해서 LLM에게 함께 전달하는 방식이다.
