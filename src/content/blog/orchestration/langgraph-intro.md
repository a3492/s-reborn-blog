...
tags: ["cu2604021138", "LangGraph", "그래프", "상태머신", "오케스트레이션"]
description: "RoundPrep 제13화. 체크리스트와 코드 사이의 간격을 메우는 도구 이야기."
read_time: 13
...
thumbnail: ""
key_takeaways:
  - "LangChain 팀이 에이전트 무한 루프 문제를 해결하려다 만든 것이 LangGraph다. 왜 그래프가 AI 흐름 설계에 적합한지, 의사의 언어로 설명한다."
  - "이 글은 설계 관점 정리이며, 프로덕션 도입 전 보안·비용·감사 요구를 별도 검토한다."
  - "태그 cu2604021138: Cursor 초안 — 프레임워크·API·병원 정책은 공식 문서를 본다."
---
> RoundPrep 제13화. 플로우 차트는 그려졌는데 코드에는 if 지옥만 쌓인다. 분기·재시도·체크포인트를 한 언어로 쓰고 싶다는 이유로 그래프를 본다.
...
**그래프는 멋이 아니라, 무한 루프와 분기를 끊어내기 위한 구조다.**

### 스테이크

에이전트 데모가 현장에서 **같은 툴을 열 번** 부르면, 그날로 신뢰는 끝이다.

### 전환점 — RoundPrep 메모

AgentExecutor만 쓰다 로그에 **어느 분기**를 탔는지 안 남아 박과장이 감사 질문을 못 넘긴다.
...
LangChain 팀은 이 문제를 해결하기 위해 아예 새로운 레이어를 만들었다. 에이전트의 흐름을 미리 명확하게 그려두는 것. 이것이 2024년 초 공개된 LangGraph다.
...
의료 현장에서 더 중요한 케이스가 있다. 사람의 승인을 기다리는 경우다.
...
이 분기 로직은 코드로 짜는 게 아니라 그래프에 그리는 것이다. 나중에 흐름을 수정할 때도 그래프를 고치면 된다. 코드 전체를 뒤질 필요가 없다.
...
> 이 글에서 나온 "사람 개입 대기" 기능을 실전에서 어떻게 설계하는지 → [Human-in-the-Loop](/blog/orchestration/human-in-the-loop/)
>
> 역할 기반으로 AI 팀을 꾸리는 다른 접근법 → [CrewAI 입문](/blog/orchestration/crewai-intro/)
...
**혜준:** “RoundPrep v0.2는 **노드와 엣지가 있는 그림**부터 커밋합니다.”

### 다음 화

그래프 안에서 에이전트가 **서로 말로 협상**하게 두려면 — [AutoGen 입문](/blog/orchestration/autogen-intro/)
