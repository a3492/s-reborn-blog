...
tags: ["cu2604021138", "컨텍스트윈도우", "메모리", "오케스트레이션", "토큰관리"]
description: "RoundPrep 제12화. 잘린 컨텍스트가 임상 브리핑에서 무엇을 의미하는지."
read_time: 12
...
**컨텍스트는 무한이 아니다. 자르면 잊는다는 전제로 설계해야 한다.**

### 스테이크

열 명 환자 요약을 한 번에 넣으면 **앞쪽 환자 정보가 증발**한다. 회진 전 브리핑이 아니라 랜덤 브리핑이 된다.

### 전환점 — RoundPrep 메모

요약→요약의 요약을 반복하다 **수치가 바뀐 채** 전달됐다는 가상 디버깅.
...
**서진:** “환자별 **1페이지 캡**을 정책으로 박읍시다.”

### 다음 화

흐름이 복잡해지면 if 지옥 대신 **그래프** 이야기가 나온다 — [LangGraph 입문](/blog/orchestration/langgraph-intro/)
