...
tags: ["cu2604021138", "디버깅", "트레이싱", "오케스트레이션", "LangSmith"]
description: "RoundPrep 제18화. 누가 누구를 불렀는지 없으면 멈춘다."
read_time: 13
...
**추적 ID 없는 오케스트레이션은, 응급실에서 원인 모르는 쇼크와 같다.**

### 스테이크

장애 나서 **재현이 안 되면** 박과장은 롤백을 외친다. 서진은 환자 브리핑을 못 받는다.

### 전환점 — RoundPrep 메모

로그에 프롬프트만 있고 **노드 ID**가 없어 하루를 태웠다.
...
**혜준:** “trace_id는 **환자·배치·요청** 삼각으로 겹칩니다.”

### 다음 화

디버깅은 일회성이 아니라 **상시 관찰**로 이어져야 한다 — [오케스트레이션 모니터링](/blog/orchestration/orchestration-monitoring/)
