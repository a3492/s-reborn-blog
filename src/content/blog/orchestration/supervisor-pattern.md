...
tags: ["cu2604021138", "슈퍼바이저", "계층구조", "멀티에이전트", "오케스트레이션"]
description: "RoundPrep 제8화. 여러 봇이 있을 때, 책임·호출 한 줄기를 잡는 패턴."
read_time: 12
...
**전문가를 여럿 두려면, 지휘자가 누구인지 한 명은 정해야 한다.**

### 스테이크

‘에이전트 둘이 알아서 협력’은 회의를 끝내지 못한다. RoundPrep은 **아침 7시에 끝나야** 한다.

### 전환점 — RoundPrep 메모

분산 채팅만 허용했더니 같은 Lab을 두 에이전트가 중복 조회했다. **중앙 슈퍼바이저** 안건이 올라온다.
...
**박과장:** “감사 관점에선 **한 개의 orchestration trace**가 있어야 합니다.”

### 다음 화

슈퍼바이저 아래 에이전트들이 **서로 뭐라고 보내는지** — [멀티 에이전트 통신](/blog/orchestration/multi-agent-communication/)
