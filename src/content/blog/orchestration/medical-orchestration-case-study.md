...
---
> RoundPrep 제16화 — 중간 정리. 화이트보드에 스티커가 빼곡하다. “병렬”, “상태”, “HITL”, “RAG”… 박과장이 묻는다. “한 장으로 말해줘요. 오늘 아침에 뭐가 어디까지 자동이에요?” 서진은 숨을 고른다. 이 글은 그 한 장을 그리기 위한 다이어그램 초안이다.
...
이 글은 회진 준비를 데이터 수집 → 이상 탐지 → 요약 → 의사 검토 → 실행 전 승인의 다단계로 나누고, 병렬·규칙/LLM 혼합·장애 시나리오까지 한 줄기로 묶는 사고 실험이다. 특정 EMR 화면 캡처는 다루지 않는다.
...
담당 환자 10명, 숙련 주치의도 준비만 30~40분. 한 번이라도 승인 없는 제안이 현장에 흘러가면 PoC는 중단이다. 반대로, 야간 K 상승 같은 신호를 규칙으로 잡을 수 있는데 LLM만 믿으면 비용·지연만 커진다.
...
가정 시나리오: 데모에선 “요약만” 잘됐다. 그런데 프로덕션 논의에서 “제안 처치”가 승인 없이 간호 호출까지 흘러갔다는 오해가 생기면 교수님은 한 번에 문을 닫는다. 그래서 위 표의 HITL 행은 장식이 아니라 제품의 경계다.
...
서진: “한 장으로 말하면, 7시에 자동으로 모이는 건 데이터와 알림이고, 처치는 교수님 승인 전엔 절대 EMR에 안 갑니다. 장애 나면 EMR만으로도 회진은 갑니다.”
